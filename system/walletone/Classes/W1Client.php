...
class W1Client {
   ...
   * @return \WalletOne\Classes\StandartMethods\W1Settings
   */
  public function setSettings($params = array()) {
    if(!empty($params)){
   ...
   */
  public function setNewParametrs($obj, $params = array()) {
    if(!empty($params)){
      foreach ($params as $key => $value) {
        if($obj instanceof W1Payments && $key == 'nameCms'){
   ...
   * @return string
   */
  public function getHtmlPayments($name, $paymentsList = array(), $nameInput = 'w1_') {
    if(!empty($this->html->filename)){
  ...
  public function resultValidation($params, $request) {
    ...
    if (!$this->result->checkSignature($request, $this->settings->signature, $this->settings->signatureMethod)) {
      $this->logger->warn(w1ErrorResultSignature);
      $this->errors[] = 'WMI_RESULT=RETRY&WMI_DESCRIPTION=' . w1ErrorResultSignature;
      return false;
    }
    return true;
  }
